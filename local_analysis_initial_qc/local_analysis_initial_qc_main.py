...
def metagenomic_checks(optional_filtering,p,calls,minorAF,coverage,scafNames,chrStarts,sampleNames,filter_site_per_sample_params,ancient_bool,force_rerun):
    """Apply metagenomic contamination filters to ancient samples.

    This combines:
      * nearby heterozygosity masking
      * low-complexity / zero-coverage genomic islands
      * excessive coverage percentile outliers
    """
    ...
    if missing_bed_files:
        ...
        raise FileNotFoundError(
            "Missing expected bedtools coverage files for one or more samples. "
            f"QC aborted before filtering. Missing files:\n{missing_bed_files_str}"
        )
    ...
    failed_metagenomic = failed_heterozygosity | failed_genomic_islands | failed_coverage_percentile
...
def main(parameter_json,force_rerun=False):
    ...
    failed_metagenomic=metagenomic_checks(optional_filtering,p,calls,minorAF,coverage,scafNames,chrStarts,sampleNames,filter_site_per_sample_params,ancient_bool,force_rerun)
