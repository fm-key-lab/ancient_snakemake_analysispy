...
rule all:
  input:
    ...
    expand("5-bedtools/{sampleID}_genome_coverage_hist.tsv.gz",sampleID=SAMPLE_ls),
    "samples_case.csv",
    "cleanUp_done.txt",

rule generate_bedtools_annotation:
    input:
        fai="/nexus/posix0/MPIIB-keylab/reference_genomes/{reference}/genome.fasta.fai",
    output:
        genome_accessions="5-bedtools/{reference}_accession_for_bedtools.txt",
        genome_annotations="5-bedtools/{reference}_file_for_bedtools.txt"
    shell:
        """
        awk -F'\t' '{print $1, "1", $2}' {input.fai} | sed 's/ /\t/g' > {output.genome_accessions}
        awk -F'\t' '{print $1, $2}' {input.fai} | sed 's/ /\t/g' > {output.genome_annotations}
        """

rule bedtools_for_metagenomics:
    input:
        genome_accessions=rules.generate_bedtools_annotation.output.genome_accessions
        genome_annotations=rules.generate_bedtools_annotation.output.genome_annotations
        bam=rules.make_data_links_ancient.output.bams,
    output:
        genome_coverage_hist='5-bedtools/{sampleID}_genome_coverage_hist.tsv.gz',
        genome_coverage_allpos='5-bedtools/{sampleID}_genome_coverage_allpos.tsv.gz',
        genome_zerocoverage='5-bedtools/{sampleID}_zerocoverage.gz',
        genome_merged_zero_covg_regions='5-bedtools/{sampleID}_merged_zero_covg_regions.gz',
    params:
        distance_for_merging=500,
        genome_coverage_hist='5-bedtools/{sampleID}_genome_coverage_hist.tsv',
        genome_coverage_allpos='5-bedtools/{sampleID}_genome_coverage_allpos.tsv',
        genome_zerocoverage='5-bedtools/{sampleID}_zerocoverage',
        genome_merged_zero_covg_regions='5-bedtools/{sampleID}_merged_zero_covg_regions',
    conda:
        "envs/bedtools.yaml", 
    shell:
        """
        samtools view -b -F 4 -q 30 bam > 0-temp/{sampleID}_filtered.bam

        bedtools genomecov -ibam 0-temp/{sampleID}_filtered.bam > {params.genome_coverage_hist}
        bedtools genomecov -bga -ibam 0-temp/{sampleID}_filtered.bam > {params.genome_coverage_allpos}
        grep -w '0$' {params.genome_coverage_allpos} > {params.genome_zerocoverage}
        bedtools merge -d {params.distance_for_merging} -i {params.genome_zerocoverage} > 0-temp/{sampleID}_temp_zerocoverage
        bedtools coverage -b 0-temp/{sampleID}_filtered.bam -a 0-temp/{sampleID}_temp_zerocoverage -hist | awk -F'\t' '{if ($4 == 0) print}' > {params.genome_merged_zero_covg_regions}
        gzip {params.genome_coverage_hist}
        gzip {params.genome_coverage_allpos}
        gzip {params.genome_zerocoverage}
        gzip {params.genome_merged_zero_covg_regions}

        rm  0-temp/{sampleID}_temp_zerocoverage 0-temp/{sampleID}_filtered.bam
        """
